...
| 层 | 接管点 | DSH 默认行为 | 本插件 |
|---|---|---|---|
| **1 · 结果裁剪** | `ctx.toolResultPruner.pruneSession` | 超过 `thresholdChars` 掐中间 | Jev 判定每个工具结果「接下来还要不要」，要的**再大也不裁**，过期的**再小也裁**（除非短于 `minCharsToPrune`）；无判定时退回 DSH 原生行为 |
...
# <某目录>/node_modules 里还需要有 @deepseek-ai/schemastery 与 @deepseek-ai/dsh-tools 两个 peer 依赖
```
